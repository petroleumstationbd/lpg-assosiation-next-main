...
import type {DownloadDocumentInput, DownloadDocumentRow, DownloadDocumentStatus} from './types';
...
function normalizeList(raw: any) {
  ...
  if (Array.isArray(raw?.data)) return raw.data as DownloadableDocumentApiItem[];
  if (Array.isArray(raw?.data?.data)) return raw.data.data as DownloadableDocumentApiItem[];
  return [];
}
...
function resolveFileUrl(item: DownloadableDocumentApiItem) {
  const direct =
    item.document ?? item.file_url ?? item.file_path ?? item.url ?? item.file ?? null;
...
function mapRow(item: DownloadableDocumentApiItem, idx: number): DownloadDocumentRow {
  return {
...
const apiDocumentsRepo: DownloadDocumentsRepo = {
  async list() {
    ...
    if (!res.ok) throw new Error(raw?.message ?? 'Failed to load downloadable documents');
  ...
  async create(input) {
    ...
    const raw = await res.json().catch(() => null);
    if (!res.ok) throw new Error(raw?.message ?? 'Failed to create downloadable document');
  ...
  async update(id, patch) {
    ...
    const raw = await res.json().catch(() => null);
    if (!res.ok) throw new Error(raw?.message ?? 'Failed to update downloadable document');
  },
  ...
  async remove(id) {
    ...
    const raw = await res.json().catch(() => null);
    if (!res.ok) throw new Error(raw?.message ?? 'Failed to delete downloadable document');
  },
};
