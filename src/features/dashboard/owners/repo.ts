...
function mapOwnerStation(row: any, index: number): OwnerStationRow {
   ...
   const status =
      ...
      '';
   const division =
      row?.division?.name ?? row?.division_name ?? row?.division ?? '';
   const district =
      row?.district?.name ?? row?.district_name ?? row?.district ?? '';
   const upazila =
      row?.upazila?.name ?? row?.upazila_name ?? row?.upazila ?? '';
   ...
   const contactPerson =
      ...
      '';
   const phone =
      row?.contact_person_phone ?? row?.phone_number ?? row?.phone ?? '';
   const stationType =
      ...
      '';
   const fuelType =
      row?.fuel_type?.name ?? row?.fuel_type ?? row?.fuel_type_name ?? '';
   const startDate =
...
function mapOwnerDetails(payload: any): OwnerDetails {
  ...
  //  console.log(payload);
   return {
      id: String(data?.id ?? ''),
      memberId: data.station_owner_id,
      photoUrl: data?.profile_image
         ? toAbs(data.profile_image)
         : DEFAULT_AVATAR,
...
export const ownersRepo: OwnersRepo = {
   async registerOwner(input) {
      ...
      const id = String(
         data?.id ?? data?.owner?.id ?? data?.station_owner?.id ?? '',
      );
   ...
   async listUnverified() {
      const rows = await apiJson<ApiOwnerRow[]>(
         '/api/station-owners/unverified',
      );
   ...
   async update(id, input) {
      ...
      if (input.fullName !== undefined)
         payload.set('full_name', input.fullName);
      if (input.phoneNumber !== undefined)
         payload.set('phone_number', input.phoneNumber);
