...
export default function MembersOverviewSection() {
   ...
   const rows = useMemo<Member[]>(() => {
      ...
      return items.map((owner, index) => {
         const stations =
            owner.gas_stations?.map(station => station.station_name) ?? [];
   ...
   const columns = useMemo<ColumnDef<Member>[]>(
      () => [
         ...
         },
         {
            ...
            minWidth: 120,
            cell: r => {
               return <span className='text-inherit'>{r.memberId}</span>;
            },
         },
      ...
      <section className='relative overflow-hidden bg-[#F4F9F4] py-14'>
         ...
         <div className='lpg-container relative'>
            <div className='mx-auto max-w-[860px] text-center'>
               {statusMessage ? (
                  <p className='mt-3 text-[11px] font-medium text-[#FC7160] md:text-[12px]'>
                     {statusMessage}
                  </p>
               ) : null}
            </div>
