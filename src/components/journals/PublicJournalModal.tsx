    ...
    async function load() {
      try {
        ...
        const res = await fetch(
          `/api/public/journals/${encodeURIComponent(journalId)}`,
          {
            cache: 'no-store',
  ...
  return (
    ...
    >
      <div className="space-y-4 p-5 text-[13px] text-[#2B3A4A]">
        {loading ? (
          <p className="text-[12px] text-[#6F8093]">Loading journal details...</p>
        ) : null}
        ...
        <div className="space-y-3">
          ...
          {details.description ? (
            <p className="text-[13px] text-[#2B3A4A]">{details.description}</p>
          ) : (
