...
export default function NoticesSection() {
   ...
   useEffect(() => {
      ...
      async function load() {
         try {
            ...
            const mapped: NoticeRow[] = list.map((n: any, idx: number) => ({
            ...
            mapped.sort((a, b) => {
               const ta = parseDate(a.publishedDate) ?? 0;
               const tb = parseDate(b.publishedDate) ?? 0;
               return tb - ta; // newest first
            });

            // re-generate SL after sort (so SL stays 01,02...)
            const sorted = mapped.map((r, i) => ({...r, sl: i + 1}));
   ...
   const newestKey = useMemo(() => {
      ...
      if (latestTs === null || latestKey === null) return null;

      const weekAgo = Date.now() - 7 * 24 * 60 * 60 * 1000;
      return latestTs >= weekAgo ? latestKey : null;
   }, [rows]);
