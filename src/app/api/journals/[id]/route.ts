...
function extractId(req: Request, params?: Record<string, any>) {
  ...
  if (!Number.isFinite(id)) throw new LaravelHttpError(400, 'Invalid journal id');
  return id;
}
...
export async function GET(req: Request, ctx: {params?: Record<string, any>}) {
  try {
    ...
    const data = await laravelFetch(`/journals/${id}`, {method: 'GET', auth: true});
...
export async function POST(req: Request, ctx: {params?: Record<string, any>}) {
  try {
    ...
    const data = await laravelFetch(`/journals/${id}?_method=PUT`, {
      method: 'POST',
      auth: true,
      body: formData,
    });
    ...
    return NextResponse.json({message: 'Failed to update journal'}, {status: 500});
  }
}
...
export async function DELETE(req: Request, ctx: {params?: Record<string, any>}) {
  try {
    ...
    const data = await laravelFetch(`/journals/${id}`, {
      method: 'DELETE',
      auth: true,
    });
    ...
    return NextResponse.json({message: 'Failed to delete journal'}, {status: 500});
  }
}
